...
def fetch_data_with_cache(start_year, end_year, end_week, options):
    all_pbp_data = []
...
def weighted_rolling_mean(series, window_size, weights):
...
def filter_stats(final_summary, stats):
    ...
    for team in final_summary['team'].unique():
        ...
        year_scaled = (years - years.iloc[0]+1)
        pre_compute = np.exp(pd.Series(range(len(years))) * np.log(1-alpha)).iloc[::-1].reset_index(drop=True)
        discount_weights = np.exp(year_scaled * np.log(season_discount)).iloc[::-1].reset_index(drop=True)

        def __windowed_average__(x):
            x=x.reset_index(drop=True)
            window = pre_compute[:len(x)] * discount_weights[:len(x)]
        ...
        team_data.loc[:, stats] = team_data[stats].expanding().apply(__windowed_average__)
...
def main():
    ...
    parser.add_option("-m", "--model", dest="model", default="CatBoost",
    ...
    parser.add_option("--predict_year", dest="predict_year", default=None,
                    help="Predict the outcome of games for the given year.  Both this and predict_week must be set to predict games")
    parser.add_option("--predict_week", dest="predict_week", default=None,
    ...
    data, stats_cols, game_cols = fetch_data_with_cache(options.start_year, options.end_year, options.end_week, options)
    ...
    if options.eval_model:
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.4) # , shuffle=False, shuffle=False
    ...
    if options.predict_year and options.predict_week:
        ...
        predict_rows = vectors.loc[(vectors['year_home'] == int(options.predict_year)) & (vectors['week_home'] == int(options.predict_week))]
        # get the model train rows
        train_rows = vectors[(vectors['year_home'] < int(options.predict_year)) | ((vectors['year_home'] == int(options.predict_year)) & (vectors['week_home'] < int(options.predict_week)))]

        X, Y = get_xy(train_rows, new_stats)
        model = models[options.model]
        model.fit(X, Y)
        y_pred = model.predict(X)
        print("Accuracy:", accuracy_score(Y, y_pred))

        X_pred, Y_pred = get_xy(predict_rows, new_stats)
